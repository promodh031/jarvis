...
from backtalk import history, signals
...
# Command session: idle until "hey <name> start the command", then
# every 2s pause is a turn, until "stop <name>".
_CMD = {"on": False, "id": None}
...
def _roster():
    ...
    if isinstance(raw, dict) and raw:
        return raw
    n = CFG.get("name") or "Jarvis"
    return {n.lower(): {"name": n,
                        "voice": CFG.get("voice") or "bm_lewis",
                        "face": "rain"}}
...
def _lev(a, b):
    """Tiny Levenshtein. Wake names get murdered by Whisper."""
    if a == b:
        return 0
    if not a:
        return len(b)
...
def _wake_prefixes():
    """Normalized prefixes that count as the wake. Phrase from config,
    plus hey/hi/ok/okay + the agent name so Whisper's usual mangling
    still fires."""
    name = _norm_speech(str(CFG.get("name") or "jarvis"))
    phrase = _norm_speech(str(CFG.get("wake_phrase") or ""))
    out = []
    if phrase:
        out.append(phrase)
    for p in (f"hey {name}", f"hi {name}", f"ok {name}",
              f"okay {name}", f"yo {name}"):
        if p not in out:
            out.append(p)
    return [p for p in out if p]


# Whisper's greatest hits. small.en turns Jarvis into "John-R-Us" / "John"
# constantly; Friday into Freddy/Fridie less often. Match the glued form
# (spaces and hyphens already stripped by _norm_speech).
_NAME_ALIASES = {
    "jarvis": {
        "jarrus", "joris", "jarvus", "jarviz", "jarvice", "jarviss",
        "jarvi", "jaervis", "charvis", "gervis",
        "john", "johnrus", "johnarus", "johnrvis", "johnervis",
        "douglas",
    },
    "friday": {
        "fridy", "fridie", "fridays", "freddy", "fready", "fryday",
    },
}


def _fuzzy_name_token(token, name):
    """True if this speech token is the agent name, including Whisper
    murder. `token` may be one word or several already-joined words."""
    name = _norm_speech(name).replace(" ", "")
    glued = _norm_speech(token).replace(" ", "")
    if not glued or not name:
        return False
    if glued == name or glued == name + "s":
        return True
    aliases = _NAME_ALIASES.get(name, set())
    if glued in aliases:
        return True
    # Extra trailing words glued on ("friday do you") are NOT the name.
    if len(glued) > len(name) + 1:
        return bool(name == "jarvis" and glued.startswith("john")
                    and len(glued) <= 10)
    if len(glued) >= 4:
        limit = 4 if len(name) >= 6 else 3
        if _lev(glued, name) <= limit:
            return True
    if name == "jarvis" and glued.startswith("john"):
        return True
    return False


def _fuzzy_name(token):
    return _fuzzy_name_token(token, CFG.get("name") or "jarvis")


def _redirect_in_rest(rest, already):
    """'hey jarvis switch to friday' / 'hey john with shift to friday'."""
    if not rest:
        return None
    for aid, spec in _roster().items():
        if aid == already:
            continue
        n = _norm_speech(spec.get("name") or aid)
        if not n:
            continue
        if (f"to {n}" in rest or f"hey {n}" in rest
                or rest.endswith(n)):
            return aid
    return None


def _strip_wake_prefix(norm):
    """If norm starts with a wake prefix, return the remainder (may be
    empty). Else None. Second-word name match is fuzzy: Whisper turns
    Jarvis into Jarrus / Joris / Jar vis."""
    ...
    parts = norm.split()
    if len(parts) < 2 or parts[0] not in ("hey", "hi", "ok", "okay", "yo"):
        return None
    if not _fuzzy_name(parts[1]):
        return None
    return " ".join(parts[2:])


def _start_tail(rest):
    """If rest is the start-command phrase, return leftover after it
    (possibly empty). Else None."""
    for p in _START_TAILS:
        if rest == p:
            return ""
        if rest.startswith(p + " "):
            return rest[len(p):].strip()
    return None


def _match_wake(norm):
    """(agent_id, leftover after hey-name) or (None, None).

    Tries 3/2/1 tokens after hey so 'hey john r us' still counts as
    Jarvis (Whisper's hyphenated hallucination of the name).
    """
    parts = norm.split()
    if len(parts) < 2 or parts[0] not in ("hey", "hi", "ok", "okay", "yo"):
        return None, None
    roster = list(_roster().items())
    for take in (3, 2, 1):
        if len(parts) < 1 + take:
            continue
        chunk = " ".join(parts[1:1 + take])
        for aid, spec in roster:
            name = spec.get("name") or aid
            if _fuzzy_name_token(chunk, name):
                rest = " ".join(parts[1 + take:])
                other = _redirect_in_rest(rest, aid)
                if other:
                    return other, None
                return aid, rest
    return None, None
...
def _is_stop_for(norm, aid):
    spec = _roster().get(aid) or {}
    name = _norm_speech(spec.get("name") or aid)
    if norm in _STOP_TAILS:
        return True
    parts = norm.split()
    if len(parts) >= 2 and parts[0] == "stop" and _fuzzy_name_token(parts[1], name):
        return True
    rest = _strip_wake_prefix(norm)
    if rest in _STOP_TAILS or rest == "stop":
        # "hey jarvis stop" only counts for that name
        woke, _ = _match_wake(norm)
        return woke in (None, aid)
    return False


def _gate_command(text):
    """Open-mic command session.

    Returns ("ignore", None) | ("start", (id, leftover)) |
    ("switch", (id, leftover)) | ("stop", None) | ("text", str).
    """
    if not str(CFG.get("wake_phrase") or "").strip() and not CFG.get("agents"):
        return "text", text
    ...
    if _CONFIRM["verb"]:
        return "text", text
    if any(_norm_speech(q) == norm or q in text.lower()
           for q in QUIT_PHRASES):
        return "text", text
    matched, rest = _match_wake(norm)
    if rest:
        tail = _start_tail(rest)
        if tail is not None:
            rest = tail
    if _CMD["on"]:
        cur = _CMD["id"]
        if cur and _is_stop_for(norm, cur):
            return "stop", None
        if matched and matched != cur:
            return "switch", (matched, rest or None)
        if matched and matched == cur:
            if rest:
                return "text", rest
            return "ignore", None
        return "text", text
    if matched is None:
        log(f"[cmd] ignored: {text}")
        return "ignore", None
    if _is_stop_for(norm, matched):
        return "ignore", None
    return "start", (matched, rest or None)
...
async def speak_reply(brain: WarmBrain, mouth: Mouth, text: str):
    ...
    try:
        ...
        if first:
            ...
            signals.static_stop()
        while mouth.speaking or not mouth._q.empty():
...
async def amain():
    ...
    mouth = Mouth()
    silence_ms = int(CFG.get("command_silence_ms") or 2000)
    ears_wake = Ears(silence_ms=480)
    ears_cmd = Ears(silence_ms=max(480, silence_ms))
    gate = make_permission_gate(mouth)
    roster = _roster()
    default_id = "jarvis" if "jarvis" in roster else next(iter(roster))
    brains = {}
    if (roster.get(default_id) or {}).get("voice"):
        CFG["voice"] = roster[default_id]["voice"]
    brain_lock = asyncio.Lock()
    signals.set_agent("", "rain", CFG.get("model") or "")

    def _agent_prompt(aid, spec):
        extra = spec.get("prompt") or ""
        if extra:
            return extra
        if aid == "friday":
            return ("You are Friday, a female AI operator. You are not "
                    "Jarvis. Never call yourself Jarvis. This conversation "
                    "is yours alone, separate from Jarvis.")
        return ("You are Jarvis. You are not Friday. This conversation "
                "is Jarvis only. Never call yourself Friday.")

    extra0 = _agent_prompt(default_id, roster.get(default_id) or {})
    brain = WarmBrain(model=model,
                      can_use_tool=gate,
                      resume_id=resume_id,
                      extra_append=extra0)
    brains[default_id] = brain

    async def _ensure_brain(aid):
        spec = roster[aid]
        async with brain_lock:
            if aid not in brains:
                log(f"[brain] connecting {spec['name']}...")
                fresh = WarmBrain(model=model, can_use_tool=gate,
                                  extra_append=_agent_prompt(aid, spec))
                await asyncio.wait_for(fresh.start(), 120)
                brains[aid] = fresh
                log(f"[brain] {spec['name']} warm")
            return brains[aid]

    async def activate(aid):
        nonlocal brain
        global NAME
        spec = roster[aid]
        brain = await _ensure_brain(aid)
        _CMD["on"] = True
        _CMD["id"] = aid
        NAME = spec["name"]
        CFG["name"] = spec["name"]
        CFG["voice"] = spec.get("voice") or CFG["voice"]
        signals.hold_listening(True)
        signals.set_agent(spec["name"], spec.get("face") or "rain",
                          brain.model)
        signals.set_state("listening")
        history.start(NAME)
        brain.session.update(turns=0, out_tokens=0, in_tokens=0, cost=0.0)
        signals.set_tokens(0, 0)
        log(f"[cmd] session on ({NAME}, voice={CFG['voice']})")

    async def _kill_speech():
        nonlocal speak_task
        mouth.shut_up()
    ...
    mode = ("hey Jarvis / hey Friday sessions"
            if (_MIC["mode"] == "open" and (CFG.get("agents")
                or str(CFG.get("wake_phrase") or "").strip()))
            else "hands-free listening (the talk key still works)"
            if _MIC["mode"] == "open"
    ...
    loop.run_in_executor(None, lambda: (warm_mouth("bm_lewis"),
                                        warm_mouth("af_heart")))
    ...
    brain.session.update(turns=0, out_tokens=0, in_tokens=0, cost=0.0)
    signals.set_tokens(0, 0)
    async def _boot_friday():
        if "friday" not in roster:
            return
        try:
            await _ensure_brain("friday")
        except Exception as e:
            log(f"[brain] friday failed: {e}")
            brains.pop("friday", None)
    asyncio.create_task(_boot_friday())
    ...
    async def _run_console_inner(verb):
        ...
        if verb == "clear":
            ...
            signals.set_agent(model=brain.model)
            ...
            if _MIC["mode"] == "open":
                ...
                mouth.say("Hands-free on. Say hey " + NAME
                          + ". Pause two seconds for an answer. Say "
                          "stop " + NAME + " when you're done.")
        elif verb == "micptt":
            resp = ""
            if _MIC["mode"] == "ptt":
    ...
    try:
        ...
        while True:
            ...
            if _MIC["mode"] == "open" and not signals.muted():
                if mic_fut is None:
                    g = _MIC["gen"]
                    if _CMD["on"]:
                        listener = ears_cmd
                        # Command sessions always barge-in: talk over
                        # the reply and it dies. Gate only yields to
                        # the talk key so we don't double-capture.
                        cmd_gate = lambda: _MIC["btn"]
                        mic_fut = loop.run_in_executor(
                            None, lambda g=g, e=listener: (g, e.listen_once(
                                gate=cmd_gate,
                                abort=lambda: (_MIC["gen"] != g
                                               or signals.muted()),
                                speaking=lambda: mouth.speaking,
                                on_barge=_barge,
                        ...
                        mic_fut = loop.run_in_executor(
                            None, lambda g=g, e=listener: (g, e.listen_once(
                                ...
                                on_partial=_on_partial)))
                waiters.add(mic_fut)
            elif mic_fut is not None:
                waiters.add(mic_fut)
            mute_poll = None
            if (_MIC["mode"] == "open" and signals.muted()
                    and mic_fut is None):
                mute_poll = loop.create_task(asyncio.sleep(0.2))
                waiters.add(mute_poll)
            done, _ = await asyncio.wait(
                waiters, return_when=asyncio.FIRST_COMPLETED)
            if mute_poll is not None and mute_poll in done:
                mute_poll.result()
                if len(done) == 1:
                    continue
            ...
            if mic_fut is not None and mic_fut in done:
                ...
                if text:
                    if signals.is_echo(text):
                        log(f"[ears] echo dropped: {text!r}")
                        continue
                    kind, payload = _gate_command(text)
                    if kind == "ignore":
                        continue
                    if kind == "start":
                        aid, leftover = payload
                        await _kill_speech()
                        await activate(aid)
                        _MIC["gen"] += 1
                        if leftover:
                            if not await handle(leftover):
                                return
                        else:
                            mouth.say("Yes, sir.")
                        continue
                    ...
                    if kind == "stop":
                        ...
                        _MIC["gen"] += 1
                        _deny_pending()
                        signals.hold_listening(False)
                        signals.set_agent("", "rain")
                        await _kill_speech()
                        log("[cmd] session off")
                        history.close()
                        signals.set_heard("")
                        mouth.say(f"Stopped. Say hey {who} when you "
                                  "want me again.")
