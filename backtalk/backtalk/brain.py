...
class WarmBrain:
    def __init__(self, model: str | None = None, can_use_tool=None,
                 resume_id: str | None = None, extra_append: str = ""):
        ...
        self._dirty = False
        self._extra_append = extra_append or ""
    ...
    async def start(self):
        ...
        def _opts(rid):
            append = DISCIPLINE
            if self._extra_append:
                append = DISCIPLINE + " " + self._extra_append
    ...
    def _tally(self, rm, count_turn=True):
        ...
        try:
            ...
            if c:
                s["cost"] += float(c)
            signals.set_tokens(s["in_tokens"], s["out_tokens"])
        except Exception:
            pass
