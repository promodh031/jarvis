...
def warm(voice=None):
    """Load a Kokoro pipeline for this voice's language letter.
    British (b…) and American (a…) are different pipelines; we keep
    both so Jarvis and Friday can swap without a reload hitch."""
    voice = voice or CFG.get("voice") or "bm_lewis"
    lang = str(voice)[0]
    with _pipe_lock:
        if lang not in _pipes:
            ...
            log(f"[mouth] loading kokoro (lang '{lang}', voice {voice})...")
...
def _stream_kokoro(text: str):
    ...
    for _, _, audio in pipe(text, voice=voice, speed=speed):
        ...
        if a.size:
            yield (np.clip(a, -1, 1.0) * 32767).astype(np.int16)
