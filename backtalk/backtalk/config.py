...
DEFAULTS = {
    ...
    "mic_mode": "ptt",
    # Open-mic wake gate. Empty = any utterance is a turn (stock
    # hands-free). A phrase like "hey jarvis" means the open mic
    # stays idle until "hey <name>, start the command". Then it
    # listens until "stop <name>", and a pause of command_silence_ms
    # ends one turn and gets the answer. Push-to-talk and typed
    # lines never need the wake.
    "wake_phrase": "",
    "command_silence_ms": 2000,
    # Two (or more) named operators. Hey <name> starts that person's
    # session and voice; stop <name> ends it. Empty = single agent
    # from "name" / "voice" above.
    "agents": {},
