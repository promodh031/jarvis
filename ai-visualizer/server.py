...
Serves the face gallery at http://127.0.0.1:8790/ and exposes:
  ...
  /state   polled by the faces (~8x/sec):
           {"state":  "idle|listening|thinking|speaking",
            ...
            "name"/"face"/"model":   who is on duty
            "heard":  str}           live transcript while you talk
  /history  matrix dialog of saved conversations
  /api/history  JSON list; /api/history/<id> is one full thread
...
HIST_DIR = Path.home() / "Library/Application Support/jarvis/conversations"
...
def mock_bus():
    ...
    return {"state": MOCK, "level": level, "samples": samples,
            ...
            "name": "", "face": "", "model": "claude-sonnet-5",
            "heard": "",
...
def read_bus():
    ...
    try:
        ...
        pass
    try:
        payload = json.loads((BUS / ".voice_agent").read_text())
    ...
    heard = ""
    try:
        payload = json.loads((BUS / ".voice_heard").read_text())
        heard = str(payload.get("text") or "")
    except (OSError, ValueError, TypeError):
        pass
    muted = False
    try:
        muted = (BUS / ".voice_mute").read_text().strip() == "1"
    except OSError:
        muted = False
    in_tokens = out_tokens = 0
    try:
        tok = json.loads((BUS / ".voice_tokens").read_text())
        in_tokens = int(tok.get("in") or 0)
        out_tokens = int(tok.get("out") or 0)
    except (OSError, ValueError, TypeError):
        pass
    return {"state": state, "level": level, "samples": samples,
            "alert": alert, "loading": loading, "rate_limits": rate_limits,
            "name": name, "face": face, "model": model, "heard": heard,
            "muted": muted, "in_tokens": in_tokens, "out_tokens": out_tokens}


def list_history():
    items = []
    HIST_DIR.mkdir(parents=True, exist_ok=True)
    old = BUS / "logs" / "history"
    if old.is_dir():
    ...
    for p in HIST_DIR.glob("*.json"):
        ...
        items.append({
            ...
            "ended": doc.get("ended"),
            "title": str(doc.get("title") or doc.get("preview") or ""),
            "preview": str(doc.get("preview") or doc.get("title") or ""),
            "n": len(turns),
        })
    items.sort(key=lambda x: x.get("started") or "", reverse=True)
    return items


def one_history(sid: str):
    if not _SAFE_ID.match(sid):
        return None
    path = HIST_DIR / f"{sid}.json"
    try:
    ...
    def do_GET(self):
        ...
        try:
            if path == "/state":
                ...
                self._static("/history.html")
            elif path == "/api/mute":
                from urllib.parse import parse_qs, urlparse
                q = parse_qs(urlparse(self.path).query)
                cur = False
                try:
                    cur = (BUS / ".voice_mute").read_text().strip() == "1"
                except OSError:
                    cur = False
                if "on" in q:
                    cur = q["on"][0] in ("1", "true", "yes")
                else:
                    cur = not cur
                try:
                    (BUS / ".voice_mute").write_text("1" if cur else "0")
                except OSError:
                    pass
                self._send(json.dumps({"muted": cur}).encode(),
                           "application/json")
            elif path == "/api/open-history":
                webbrowser.open(f"http://127.0.0.1:{PORT}/history")
                self._send(b'{"ok":true}', "application/json")
            elif path == "/api/history":
                self._send(json.dumps(list_history()).encode(),
                           "application/json")
